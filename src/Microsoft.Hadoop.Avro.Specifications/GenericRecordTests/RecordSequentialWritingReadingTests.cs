...
namespace Microsoft.Hadoop.Avro.Specifications
{
    ...
    [TestClass]
    public sealed class RecordSequentialWritingReadingTests
    {
        ...
        public void RecordSequentialWritingReading_SimpleRecord()
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var streamWriter = AvroContainer.CreateGenericWriter<AvroRecord>(StringSchema, stream, Codec.Null))
                {
                    using (var writer = new SequentialWriter<AvroRecord>(streamWriter, 24))
                    {
        ...
        [TestMethod]
        [TestCategory("CheckIn")]
        public void RecordSequentialWritingReading_NestedRecord()
        {
            const string StringSchema = @"{
                                        ""type"":""record"",
        ...
        [SuppressMessage("Microsoft.Usage", "CA2202:Do not dispose objects multiple times",
            Justification = "We should handle multiple dispose properly.")]
        public void RecordSequentialWritingReading_RecursiveRecord()
        {
            const string StringSchema = @"{
                                ""type"":""record"",
