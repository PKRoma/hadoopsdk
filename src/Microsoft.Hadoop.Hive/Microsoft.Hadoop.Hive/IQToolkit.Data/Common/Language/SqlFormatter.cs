    ...
    public class SqlFormatter : DbExpressionVisitor
    {
        ...
        protected override Expression VisitSelect(SelectExpression select)
        {
            ...
            else
            {
                ...
                this.VisitMap(select.Map, select.Columns, select.From);
            }
            if (select.From != null)
            {
        ...
        private Expression VisitMap(MapExpression map, ReadOnlyCollection<ColumnDeclaration> columns, Expression from)
        {
            var keyColumns = "";
            if (map.Mode == "REDUCE")
            {
                this.Write("REDUCE ");
                keyColumns = string.Join(",", ClusterByFinder.FindClusterByKeyColumns(from));
            }
            else
            {
                this.Write("MAP ");
            }
            this.WriteColumns(columns.Select(c => new ColumnDeclaration("", c.Expression, c.QueryType)).ToReadOnly());
            this.WriteLine(Indentation.Same);
            var usingStatement = string.Format("USING './HiveDriver.exe {0} {1} {2} {3} {4}'",
                map.Mode, System.IO.Path.GetFileName(map.AssemblyFullPath), map.ClassName, Base64Codec.EncodeTo64(map.MethodName), keyColumns);
