    ...
    public class PowerShellCmdletSteps
    {
        ...
        private Dictionary<string, CmdletHouse> LocateCmdlets()
        {
            ...
            if (!ScenarioContext.Current.TryGetValue(CmdTestingConstants.Cmdlets, out cmdlets))
            {
                ...
                foreach (var type in typeof(NewAzureHDInsightClusterCmdlet).Assembly.GetTypes())
                {
                    if (typeof(Cmdlet).IsAssignableFrom(type))
                    {
                        var cmdletAttribute = type.GetCustomAttributes(typeof(CmdletAttribute), false).FirstOrDefault().As<CmdletAttribute>();
                        if (cmdletAttribute.IsNotNull())
                        {
                            ...
                            foreach (var propertyInfo in type.GetProperties())
                            {
                                ...
                                // Get all the Aliases
                                var paramAliases = propertyInfo.GetCustomAttributes(typeof(AliasAttribute), false).Cast<AliasAttribute>();
                                if (paramAliases.Any())
                                {
                                    aliases.AddRange(paramAliases.SelectMany(a => a.AliasNames));
                                }
                                // Get all the ParameterAttributes
                                var paramAttribs = propertyInfo.GetCustomAttributes(typeof(ParameterAttribute), false).Cast<ParameterAttribute>();
                                if (paramAttribs.Any())
                                {
        ...
        public void ThenNoParameterLacksEitherAGetterOrASetter()
        {
            ...
            foreach (var parameterSet in cmdlet.ParameterSets)
            {
                ...
                {
                    Assert.IsTrue(parameterHouse.Property.GetGetMethod().IsNotNull(),
                                  "The parameter '{0}' lacks a getter",
                                  parameterHouse.FullName);
                    Assert.IsTrue(parameterHouse.Property.GetSetMethod().IsNotNull(),
