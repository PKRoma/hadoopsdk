...
namespace Microsoft.Hadoop.MapReduce.Test
{
    ...
    public class DifferentEncodings
    {
        private void TestEncoding(Encoding encoding)
        {
            ...
            if (HdfsFile.Exists(folder))
            {
                HdfsFile.Delete(folder);
            }
            const string input =
@"PureFoo
PureFoo
Bar";
            HdfsFile.WriteAllBytes(folder + "/in", encoding.GetBytes(input));
            HadoopJobExecutor.Execute<Mapper,Reducer>(MakeConfig(folder));
            var output = HdfsFile.EnumerateDataInFolder(folder + "/out", maxLines: 5000).OrderBy(s => s).ToList();
