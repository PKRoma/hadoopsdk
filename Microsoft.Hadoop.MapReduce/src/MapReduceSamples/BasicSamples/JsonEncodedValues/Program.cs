...
namespace JsonEncodedValues
{
    public class Program
    {
        ...
        {
            CreateInput();
            HadoopJobExecutor.ExecuteJob<JsonEncodedValuesJob>();
        ...
        private static void CreateInput()
        {
            ...
            if (HdfsFile.Exists(JsonEncodedValuesJob.s_inputFolderHDFS))
            {
                HdfsFile.Delete(JsonEncodedValuesJob.s_inputFolderHDFS);
            }

            HdfsFile.MakeDirectory(JsonEncodedValuesJob.s_inputFolderHDFS);
            HdfsFile.WriteAllLines(JsonEncodedValuesJob.s_inputFileHDFS, new string[] {"Eastern Standard Time", "Pacific Standard Time"});
        }
    }
