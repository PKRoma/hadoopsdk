...
namespace ReduceSideJoin
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            HadoopJobExecutor.Execute<ReduceSideJoinMapper, ReduceSideJoinReducer>(config);

            Console.WriteLine();
            Console.WriteLine("==========================");
            Console.WriteLine("Output:");
            foreach (string s in HdfsFile.EnumerateDataInFolder(ReduceSideJoinJob.s_OuputFolder, 20))
            {
                Console.WriteLine(s);
            }
        }
        ...
        private static void CreateInput()
        {
            ...
            if (HdfsFile.Exists(ReduceSideJoinJob.s_InputFolder))
            {
                HdfsFile.Delete(ReduceSideJoinJob.s_InputFolder);
            }

            HdfsFile.MakeDirectory(ReduceSideJoinJob.s_InputFolder);
            string inputFilePath_a = HdfsPath.Combine(ReduceSideJoinJob.s_InputFolder, "a");
            HdfsFile.WriteAllLines(inputFilePath_a, 
                new string[] { 
                    " A\t1\tMike", 
                    " A\t2\tBob", 
                    " A\t3\tFrank", 
            ...
            HdfsFile.WriteAllLines(inputFilePath_b,
                new string[] { 
                    " B\t3\tFrank-Home", 
                    " B\t1\tMike-Home", 
                    " B\t1\tMike-Mobile", 
