...
namespace ParameterSweep
{
    public class Program
    {
        ...
        {
            //create input
            HdfsFile.WriteAllLines(ParameterSweepJob.s_inputFileHDFS, Enumerable.Range(0, 10).Select(x => x.ToString()));

            //run
            HadoopJobExecutor.ExecuteJob<ParameterSweepJob>();

            foreach (string line in HdfsFile.EnumerateDataInFolder(ParameterSweepJob.s_outputFolderHDFS, 20))
            {
    ...
    public class ParameterSweepMapper : MapperBase
    {
        public override void Map(string inputLine, MapperContext context)
        {
            ...
            HdfsFile.WriteAllLines(outputFileHdfs, new[] { string.Format("{0}\t{1}", inputValue, sqrt) });
